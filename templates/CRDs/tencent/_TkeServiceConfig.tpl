{{- /*
reference:
- https://cloud.tencent.com/document/product/457/45490
- https://cloud.tencent.com/document/product/457/45700
*/ -}}
{{- define "crds.tencent.TkeServiceConfig" -}}
  {{- $_ := set . "_kind" "TkeServiceConfig" }}

  {{- nindent 0 "" -}}apiVersion: cloud.tencent.com/v1alpha1
  {{- nindent 0 "" -}}kind: {{ ._kind }}
  {{- nindent 0 "" -}}metadata:
    {{- include "definitions.ObjectMeta" . | trim | nindent 2 }}

  {{- $__spec := include "crds.tencent.TkeServiceConfig.TkeServiceConfigSpec" . | fromYaml }}
  {{- if $__spec }}
    {{- nindent 0 "" -}}spec:
      {{- toYaml $__spec | nindent 2 }}
  {{- end }}
{{- end }}
